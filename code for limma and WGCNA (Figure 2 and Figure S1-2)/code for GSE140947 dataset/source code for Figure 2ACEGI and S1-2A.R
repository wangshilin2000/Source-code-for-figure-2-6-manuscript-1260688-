...
# export Clustering dendrogram as Figure S1A

pdf("FigureS2A.pdf",8,6)
par(mfrow = c(1,2));
cex1 = 0.9;
...
# export Cluster Dendrogram as Figure 2E

clinical=read.table("clinical140947.txt",header = T,sep = "\t",row.names = 1)
rownames(clinical)
...
write.table(MMGS, "MMGS140947.txt", sep = "\t", quote = F)
...
a=which(merge$colors=="brown")  
b=datExpr[,a]
c=colnames(b)
write.table(c,"WGCNAgene140947.txt",quote = F,row.names = F,col.names = F)

# export gene significance plot as Figure 2I
