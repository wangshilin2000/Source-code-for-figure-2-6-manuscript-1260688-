...
write.table(geneCoef, file="geneCoef7084.xls", sep="\t", quote=F, row.names=F)
write.table(file="lassoset7084.txt",lassoGene,sep="\t",quote=F,col.names=F,row.names=F) 

pdf("Figure4FG.pdf",height = 5,width = 7)
...
pdf("Figure4J.pdf",width = 10,height = 5)
PlotErrors(errors, no.info=no.info) 
dev.off()

pdf("Figure4K.pdf",width = 10,height = 5)
...
bdf=randomForest(as.factor(group)~.,
                 ...
                 ntree=20)
bdf
pdf(file="Figure4M.pdf", width=6, height=6)
...
pdf(file="Figure4O.pdf", width=6, height=6)
print(p1)
dev.off()

bdfGenes=arrange(importancedata,desc(importance))
write.table(bdfGenes, file="bdfGenes7084.xls", sep="\t", quote=F, col.names=T, row.names=F)
